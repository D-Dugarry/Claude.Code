...
- **`Hoja1.cls`, `Módulo_Filtro_Avanzado_Prueba.bas`** — residuales/plantilla por defecto de Excel o pruebas sueltas, sin renombrar; no forman parte de la arquitectura activa. (`Módulo1/3/4/5.bas` se eliminaron en 2026-09-19.)
...
- **`M00_Ini_Var_APP.bas`** — sin lógica: es el **esquema de datos central**, define como `Public Const` el índice de columna de cada tabla clave (`BD_*` en `Prog_BD`, `G04_*` en `Prog_LsGes04`, `CLiq_*` en la Liquidación, `CSol_*` en Solicitudes, `DefC_*`, `InfRec_*`). Referencia obligada antes de tocar cualquier rutina que lea/escriba columnas de una tabla.
- **`BD_*` (Tb_BDatos, 59 col.) y `G04_*` (Tb_LsGes04, 48 col.) son esquemas DISTINTOS** (separados en la Fase 4, 2026-09-23). Solo coinciden las col. 1..39; en `G04_` la 40..43 son de trabajo (`Incidencias`, `H_Incidencias`, `EP_Ctrl`, `EP_GestReg`, que se mapean a `BD_Incidencias`..`BD_EP_GestReg` = 51..54) y la 44..48 los flags de tipo de recibo de `M05` (no pasan a `Prog_BD`). Sobre `Lo_Ges04` o el LSGES04 recién abierto usar **siempre `G04_*`**; su definición de columnas es `Prog_DefCol_G04` (no `Prog_DefCol_BD`). Para copiar filas de `Lo_Ges04` a una tabla con estructura BD (`Tb_BDatos`, `Tb_Duplic`) usar `Rut_Lo_G04_Filtered_Copy_a_BD` (en `M07`), **nunca** `Rut_Lo_DataBodyRange_Filtered_Copy`, que copia por posición. `Fnc_G04_Alineada_con_BD` (en `M07`) detiene el proceso si alguien desalinea las col. 1..39. Si se cambia `Tb_LsGes04`, hay que tocar a la vez las constantes de `M00`, la tabla y `Tb_DefCol_G04` (con `Compare` = FALSO en las col. 40..48).
- **Duplicados (`M02_Manage_Duplicates` → `Tb_Duplic`), reglas acordadas con el usuario (2026-09-23).** Cada LSGES04 trae TODOS los recibos del curso, así que los mismos duplicados reaparecen en cada descarga. De cada serie con la misma `Ref` se conserva la última del LSGES04 — **no hay criterio para elegir cuál (lo confirma el usuario), no buscarle uno**. Tras añadir los finalistas a `Tb_Duplic`, `Rut_Duplic_Depurar` borra: filas sin `Ref` (`RpVacio`), `Ref` que en esta descarga ya no sale duplicada (`RpNoSale`) y versiones antiguas **idénticas en todas las col. con `DefC_Compare`** a otra posterior de su `Ref` (`RpIdem`). Si cambia algún valor comparado es otro duplicado y se conservan ambas versiones (histórico, a propósito).
