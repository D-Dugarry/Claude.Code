...
- `Convert_PDF_Liquid_EP_to_Excel.spec` (PyInstaller) añadido, siguiendo el patrón de `Corrector_Orientacion_PDF.spec` (`collect_all('pymupdf')` + `datas=[('SnailSystem.png', '.')]`; sin `collect_all` para `openpyxl`/`Pillow`, no lo necesitan). Compilado y verificado que el `.exe` arranca (`python -m PyInstaller Convert_PDF_Liquid_EP_to_Excel.spec --noconfirm`). `build/`, `dist/` y `*.exe` están en `.gitignore`: no se versionan, solo el `.spec`.
