Attribute VB_Name = "M_215_TPV_Mail_Do_Devol"
Option Explicit

'==================================================================================================
' M_215_TPV_Mail_Do_Devol
'
' Envia un e-mail solicitando que se realice la Devolucion (DEV) de la liquidacion TPV.
'
' DESTINATARIO : ingresos@gcloud.ua.es (cuenta de gestion de ingresos de la UA)
' ADJUNTO      : ninguno
' TABLA HTML   : cabecera de la liquidacion (H_Liq_TPV, rango a1:h9)
'
' DIFERENCIA con M_115_CTA_Mail_Do_Devol:
'   TPV: ingreso en cuenta general de la UA -> hay que hacer retencion del 100%
'   CTA: ingreso en cuenta de JyC           -> NO hay retencion
'
' REQUIERE:
...
'   "TPVMailDoDev" para copiarlo manualmente. Resuelto en 2026 con App Password (CDO).
'==================================================================================================


'--------------------------------------------------------------------------------------------------
' Rut_Email_TPV_do_Devol
'   Construye y envia el e-mail de solicitud de DEV para una liquidacion TPV
'--------------------------------------------------------------------------------------------------
Public Sub Rut_Email_TPV_do_Devol()
    ...
    '- En 2024 con el 2FA fallo y se arreglo en 2026, mientras tanto se uso esta opcion.
        'Const SheetMail       As String = "TPVMailDoDev"
        'Sheets(SheetMail).Range("a10:h18").ClearContents
        'With Sheets(SheetMail)
        '    .Range("b1") = Asunto
        '    .Range("e4") = "JyC Solicitud " & UCase(Range("Liq_Solicitud")) & ", Liquidación nº " & Range("Liq_Núm") & "-DEV, JyC " & Range("Liq_Siglas")
        ...
        '    .Range("b8") = "Liq_JyC_" & Format(Date, "yyyy") & "__" & Range("Liq_Siglas") & "_" & Range("Liq_Núm") & "-DEV__" & Range("Liq_Nombre")
...
End Sub     '- Rut_Email_TPV_do_Devol ---
