...
'===================================================================================================================================
Sub Rut_Email_TPV_RDT_Done()
    Const SheetMail     As String = "TPVMailRDTDone"
...
'    Set Rng = Rng.SpecialCells(xlCellTypeVisible)
    ...
    Asunto = "JyC RDT_Done, Solicitud " & UCase(Range("Liq_Solicitud")) & ", Liquidación nº " & Range("Liq_Núm") & ", " & Range("Liq_Siglas") & " - ¡Efectuada!"
    With Sheets(SheetMail)
        .Range("b1") = Asunto
        .Range("i2") = Range("Liq_Email") & ", facturacion@ua.es"
        ...
        .Range("c8") = Format(Range("Liq_imp"), "#,##0.00 €")     '- Format(Range("Liq_RDT_DI_Imp"), "#,##0.00 €")
        .Range("c4:c8").WrapText = False
        .Range("a10:a" & .Cells(.Rows.Count, "A").End(xlUp).Row).EntireRow.Delete     '- ClearContents
        Rng.Copy (.Range("a10"))
    ...
    Mail_Body = "<div style='line-height:1.8; font-family:Arial, sans-serif; font-size:14px;'>" & _
                ...
                "</tr>" & "<tr>" & _
                    ...
                    "<td><b><font color='red'>" & Format(Range("Liq_DI_Imp"), "#,##0.00") & " &euro;" & "</font></b></td>" & _
                "</tr>" & _
                "</table><br>" & _
