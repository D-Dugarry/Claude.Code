Attribute VB_Name = "M_125_CTA_Mail_DEV_Done"
Option Explicit

'==================================================================================================
' M_125_CTA_Mail_DEV_Done
'
...
'
' DIFERENCIA con M_120_CTA_Mail_RDT_Done:
'   M_120 = confirmacion de RDT (Redistribucion de ingreso)
'   M_125 = confirmacion de DEV (Devolucion de ingreso, liquidacion con sufijo -DEV)
'
' DIFERENCIA con M_225_TPV_Mail_DEV_Done:
...
'   "CTAMailDevDone" para copiarlo manualmente. Resuelto en 2026 con App Password (CDO).
'==================================================================================================


'--------------------------------------------------------------------------------------------------
' Rut_Email_CTA_DEV_Done
'   Construye y envia el e-mail de confirmacion de DEV realizada para una liquidacion CTA
'--------------------------------------------------------------------------------------------------
Public Sub Rut_Email_CTA_DEV_Done()
    ...
    '- En 2024 con el 2FA fallo y se arreglo en 2026, mientras tanto se uso esta opcion.
        'Const SheetMail     As String = "CTAMailDevDone"
        'Sheets(SheetMail).Range("a10:h16").ClearContents
        'With Sheets(SheetMail)
        '    .Range("b1") = Asunto
        '    .Range("h2") = Range("Liq_Email") & ", facturacion@ua.es"
        '    .Range("c4") = "JyC Solicitud " & UCase(Range("Liq_Solicitud")) & ", Liquidación nº " & Range("Liq_Núm") & "-DEV, JyC " & Range("Liq_Siglas")
        '    .Range("c5") = Range("Liq_Nombre")
        '    .Range("c6") = "Liquidación nº " & Range("Liq_Núm") & "-DEV"
        '    .Range("c7") = Format(Range("Liq_RDT_DI_Imp"), "#,##0.00 €")
        '    .Range("c4:c7").WrapText = False
        '    .Range("a9:a" & .Cells(.Rows.Count, "A").End(xlUp).Row).EntireRow.Delete     '- ClearContents
        '    Rng.Copy (.Range("a9"))
...
End Sub     '- Rut_Email_CTA_DEV_Done ---
